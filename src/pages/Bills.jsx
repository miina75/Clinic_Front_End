...
export default function Bills() {
  ...
  return (
    <div>
      <h2 className="text-xl font-bold text-gray-800 mb-6">Bills</h2>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 text-left text-xs text-gray-400 uppercase">
                <th className="pb-3 font-medium">BillId</th>
            ...
            <tbody>
              {bills.map((bill, i) => (
                <tr key={bill.billId} className="border-b border-gray-50 last:border-0 hover:bg-gray-50">
                  <td className="py-3 text-gray-400">{bill.billId}</td>
                  <td className="py-3 text-gray-700 font-medium">{bill.visitId}</td>
                  <td className="py-3 text-gray-700 font-semibold">${bill.amount}</td>
                  <td className="py-3">
                    <span className={`rounded-full px-3 py-1 text-xs font-medium ${statusColors[bill.paymentStatus] ?? 'bg-gray-100 text-gray-600'}`}>
                      {bill.paymentStatus}
                    </span>
                  </td>
                  <td className="py-3 text-gray-600">{bill.billDate?.split('T')[0]}</td>
                  <td className="py-3">
